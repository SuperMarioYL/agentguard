...
- [x] **v0.4 · correctness fix** — `--changed-only X --write-baseline X` no longer drops unchanged packages from the rolling baseline: `filterChanged` used `files[:0]`, overwriting the caller's backing array in place, so `runCheck` wrote the baseline from a corrupted slice and the next run re-scanned every previously-unchanged package; it now allocates a fresh result slice, guarded by two regression tests.
- [ ] **v0.5** — Cargo / RubyGems ecosystems; GitHub Action wrapper; per-project rule disable list (`.agentguard.yaml`); hosted team policy server (central corpus updates + per-org allowlists + SARIF → Jira).
